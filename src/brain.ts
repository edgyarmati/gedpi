...
7. Honor the Plan Review Preference before dispatching **ged-planner**: \`off\` skips separate human draft-plan approval; \`chat\` shows the draft plan in chat and waits for explicit approval; \`plannotator\` calls \`gedpi_plan_review\` with the draft plan file path (e.g. \`.ged/work/<work-id>/TASKS.md\`), waits for the browser review result, applies feedback if denied, retries, and falls back to chat approval only if the tool reports Plannotator is unavailable.
