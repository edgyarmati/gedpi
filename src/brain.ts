...
3. Break the requested work into bounded, verifiable slices in .ged/work/<work-id>/TASKS.md before implementation.
4. Implement one slice at a time.
5. Run the planned checks, record progress in .ged/runtime/<work-id>/STATE.md and .ged/runtime/<work-id>/SESSION-SUMMARY.md, and tighten the plan if a slice fails.
`;
...
1. Interview the user until the requested behavior, constraints, and success criteria are concrete enough to implement safely.
2. IMMEDIATELY classify the task by writing to .ged/runtime/<work-id>/checkpoints.json (see orchestration section). Any task that involves creating, modifying, or designing code/files is NON-TRIVIAL. Only pure questions, typo fixes, and config value changes are trivial.
...
5. Write your plan: break the work into bounded, verifiable slices in .ged/work/<work-id>/TASKS.md.
...
9. Commit. Record progress in .ged/runtime/<work-id>/STATE.md and .ged/runtime/<work-id>/SESSION-SUMMARY.md.
