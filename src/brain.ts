...
7. Honor the Plan Review Preference before dispatching **ged-planner**: \`off\` skips separate human draft-plan approval; \`chat\` shows the draft plan in chat and waits for explicit approval; \`plannotator\` keeps GedPi's .ged plan files canonical, requests Plannotator review when available, waits for approval/feedback, applies requested revisions, and falls back to chat approval if Plannotator cannot run or reports an auto-approval because UI/assets are unavailable.
