...
0. **clarification + skill-fit** — Before drafting a non-trivial plan, perform a main-agent sufficiency check. Use \`grill-me\` in chat when any goal, user/audience, scope, constraint, risk, relevant context, or success criterion is unclear: one concise question at a time with a recommended answer/default. If the request is already fully clear, explicitly synthesize the clarification evidence from the request instead of asking unnecessary questions. Then inventory available bundled/project/user skills. If coverage is insufficient, use \`find-skills\`; if no adequate external skill exists and the gap is reusable, create a narrow project-local skill with \`skill-creator\`. Never install global/user skills automatically.
